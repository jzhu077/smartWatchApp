/**
 * Created by Frankyang on 22/05/2016.
 */
...
if (Swipe_Right) {
    ...
    if(Layer==3){
       ...
       // radar_search_info('restaurant','5000');
    }
    if(Layer==4)
        radar_search('restaurant','5000');
    if(Layer==5)
        radar_search_info('restaurant','5000');
...
//
    ...
    function radar_search_info(keyword,radius) {
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(function (position) {
                var pos = {
                    lat: position.coords.latitude,
